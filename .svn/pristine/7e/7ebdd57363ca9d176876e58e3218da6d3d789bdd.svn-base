...
<script type="text/javascript">
	...
	function fn_LAccCdChk(saveyn) {
		...
		if(!fn_Validate()){
			$("#chkLCd").val("0");
			return "N";
			//빈 값 : false -> true 변동, N 반환
			//빈 값 X :true -> false 변동, else 절
		} else {
				//4자리 숫자 유효성검사
	...
	function fn_SAccDtCdChk(saveyn) {
		...
		if(!fn_SValidate()){
			return "N";
			//값이 없으면 fn_SValidate -> false -> !fn_SValidate() - true : 값 없을 때  return "N" ---> fn_bigInsert -> else 구문
	...
	function fn_accAcmSList(currentPage) {
		...
		var resultCallback = function(data) {
			...
			//선택한 계정대분류에 맞는 계정상세 목록과 현재 계정대분류 목록 페이지를 함께 계정상세 목록페이지에 넘김
	...
	function fn_bigInsert() {
		...
		if(fn_LAccCdChk("Y") == "N") {//빈 값, 4자리 숫자 유효성검사 실패시
			return;
		} else {
	...
	function fn_smallInsert() {
		...
		if(fn_SAccDtCdChk("Y") == "N") {//빈 값, 4자리 숫자 유효성검사 실패시
			return;
		} else {
				...
				<li class="contents">
					...
					<div class="content">
						...
						<p class="Location">
							...
							<a href="../accAcm/accManagement.do" class="btn_set refresh">새로고침</a>
						</p>
                        
						<p class="conTitle">
							<span>계정과목 관리</span> 
							<span class="fr"> 
								<a href="javascript:fn_openpopup();" name="modal">
						...
						<div style="height: 30px;  text-align: center; border: 3px solid #c0c0c0;  border-radius: 10px; padding-top: 13px; padding-bottom: 13px; margin-bottom: 20px; margin-top: 20px;">
							...
							<div style="display: inline-block; border: 3px solid #c0c0c0; padding: 5px 40px 5px 40px; font-weight: bold; font-size: 15px; background: #c0c0c0; border-radius: 2px;">
								<a href="" id="btnSearch" name="btn">
									<span style=" color: black;">검&nbsp;색</span>
								</a>
							</div>
						</div>	
