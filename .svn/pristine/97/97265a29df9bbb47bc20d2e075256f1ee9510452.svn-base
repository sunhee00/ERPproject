...
<script type="text/javascript">
	...
	$(function() {
		...
		// combo type(combo box 종류),  combo_name, type(기본값  all : 전체   sel : 선택) , "", "" 
		selectComCombo("acc","acccombo","all","","");  
		
		// combo box 종류  accd : 상세 회계계정   acccombo 변경시 계정 상세 재조회 event   조회 대상 테이블  tb_dt_sbject 
		// combo type(combo box 종류),  combo_name, type(기본값  all : 전체   sel : 선택) , 선택된 상위 계정코드, ""
		$('#acccombo').change(function() {   
			selectComCombo("accd","accdcombo","all",$('#acccombo').val(),"");   
		});
		
		// combo box 종류  cli : 거래처    조회 대상 테이블  tb_clnt   
		// combo type(combo box 종류),  combo_name, type(기본값  all : 전체   sel : 선택) , "", "" 
		selectComCombo("cli","clicombo","all","","");  
	...
	function fn_acSearchlist(pagenum) {
		...
		var listcollabck = function(returnvalue) {
			...
			//$("#acSearchlist") 위치에 회계전표 목록 그리드 삽입
	...
	//모달 팝업 열 때 조회한 상세내용 위치에 맞게 보여주기
	function popupinit(object) {
		...
		if(object == "" || object == null || object == undefined) {
			...
			$("#bno").val(object.budget_no);			//회계전표 번호	
			$("#person").val(object.name);				//담당자
			$("#date").val(object.pdate);				//전표 날짜
			$("#clientNm").val(object.client);			//거래처 이름
			$("#inoutNm").val(object.acnt_sbjct_inout); //지출구분
			...
			$("#Eno").val(object.expen_report_no);		//지출번호
			$("#Suname1").val(object.acnt_dt_sbjct_name);//계정과목이름
			$("#Suname2").val("현금");
			$("#E1").val(outamt);
			$("#R1").val(inamt);
	...
	function fn_selectone(no) {
		...
		var selectoncallback = function(returndata) {	
			...
			console.log("하하호호" +  returndata ); //[object Object]
			console.log( JSON.stringify(returndata) ); //내용물 출력
				...
				<li class="contents">
					...
					<div class="content">
						...
						<p class="Location">
							...
							<span class="btn_nav bold">회계</span> <span class="btn_nav bold">회계전표</span> 
							<a href="../accAcs/accountSearch.do" class="btn_set refresh">새로고침</a>
						</p>
						...
						<div style="border : solid 3px #c0c0c0; height: 150px; border-radius: 10px;">
							...
							<div style="margin-top: 15px; margin-bottom : 15px; text-align: center; ">
								...
								<label for="accdcombo" style="margin-left: 15px; font-size: 15px; font-weight: bold">계정 상세&nbsp;&nbsp;</label>
								 <select id="accdcombo" name="accdcombo" style="width: 100px; height: 25px;" ></select>
								 
								<label for="clicombo" style="margin-left: 15px; font-size: 15px; font-weight: bold">거래처명&nbsp;&nbsp;</label>
								<select id="clicombo" name="clicombo" style="height: 25px; width: 120px;" ></select> 
	...
	<div id="layer1" class="layerPop layerType2" style="width: 1000px; height: 500px;">
		<dl>
			...
			<dd class="content">
			  ...
			  <a href=""	class="btnType gray"  id="btnClose" name="btn" style="margin-left: 42.5%; margin-top: 30px; "><span>닫기</span></a>
			</dd>
		</dl>
