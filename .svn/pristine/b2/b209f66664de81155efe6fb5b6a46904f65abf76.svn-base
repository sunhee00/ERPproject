package kr.happyjob.study.accAcm.model;

public class accAcmModel {


	private int acnt_sbject_cd; //계정대분류
	private String acnt_sbject_name; //계정대분류명
	private int acnt_dt_sbject_cd; //계정상세코드
	private String acnt_dt_sbjct_name; //계정상세명
	private String acnt_sbjct_inout; //계정 수입/지출 구분
	
	
	public int getAcnt_sbject_cd() {
		return acnt_sbject_cd;
	}
	public void setAcnt_sbject_cd(int acnt_sbject_cd) {
		this.acnt_sbject_cd = acnt_sbject_cd;
	}
	public String getAcnt_sbject_name() {
		return acnt_sbject_name;
	}
	public void setAcnt_sbject_name(String acnt_sbject_name) {
		this.acnt_sbject_name = acnt_sbject_name;
	}
	public int getAcnt_dt_sbject_cd() {
		return acnt_dt_sbject_cd;
	}
	public void setAcnt_dt_sbject_cd(int acnt_dt_sbject_cd) {
		this.acnt_dt_sbject_cd = acnt_dt_sbject_cd;
	}
	public String getAcnt_sbjct_inout() {
		return acnt_sbjct_inout;
	}
	public void setAcnt_sbjct_inout(String acnt_sbjct_inout) {
		this.acnt_sbjct_inout = acnt_sbjct_inout;
	}
	public String getAcnt_dt_sbjct_name() {
		return acnt_dt_sbjct_name;
	}
	public void setAcnt_dt_sbjct_name(String acnt_dt_sbjct_name) {
		this.acnt_dt_sbjct_name = acnt_dt_sbjct_name;
	}
	
	

	
	
	
}
